...
DROP TABLE LINK_BAND CASCADE CONSTRAINTS;
DROP TABLE link_blog CASCADE CONSTRAINTS;
DROP TABLE LINK_JISIKIN CASCADE CONSTRAINTS;
...
DROP TABLE LINK_TV CASCADE CONSTRAINTS;
...
DROP TABLE LINK_USERS CASCADE CONSTRAINTS;
...
(
	...
	review_no number(7,0) NOT NULL,
	reputation_like number(4,0) DEFAULT 0 NOT NULL,
	reputation_dislike number(4,0) DEFAULT 0 NOT NULL,
	USER_ID varchar2(50) NOT NULL,
...
(
	...
	tv_no number(5,0) NOT NULL,
	comments_contents varchar2(600) NOT NULL,
	create_date date NOT NULL,
	USER_ID varchar2(50) NOT NULL,
	PRIMARY KEY (comment_no)
);
...
ALTER TABLE BAND_WRITE
	...
	REFERENCES LINK_BAND (band_no)
;


ALTER TABLE blog_board_likes
	ADD FOREIGN KEY (blog_no)
	REFERENCES link_blog (blog_no)
;
...
ALTER TABLE JISIKIN_ANSWER
	...
	REFERENCES LINK_JISIKIN (JISIKIN_NO)
;
...
ALTER TABLE LINK_BAND
	ADD FOREIGN KEY (USER_ID)
	REFERENCES LINK_USERS (USER_ID)
;


ALTER TABLE link_blog
	ADD FOREIGN KEY (USER_ID)
	REFERENCES LINK_USERS (USER_ID)
;


ALTER TABLE LINK_JISIKIN
	ADD FOREIGN KEY (USER_ID)
	REFERENCES LINK_USERS (USER_ID)
;
...
COMMENT ON COLUMN LINK_BAND.band_name_open_yn IS '밴드의 이름은 오픈되지만 게시물은 멤버만 공개';
COMMENT ON COLUMN LINK_BAND.band_join_apply_yn IS '밴드 이름만 공개시 사용할 컬럼';
COMMENT ON COLUMN link_blog.blog_no IS '블로그';
COMMENT ON COLUMN link_blog.blog_title IS '블로그';
